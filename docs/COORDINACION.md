...
| `ui/ronda-3-clutter-y-transiciones` | `agente-ui` | Ronda 3 tarea 1, limpieza de z-index/solapes, transiciones más rápidas, ficha cerrable al tocar. **Incluye una anulación explícita de ADR-001** — ver sección propia abajo. El encabezado ya NO se repliega de forma reactiva durante el gesto: ese enfoque (`movestart`/`moveend`) causó parpadeo real en vivo dos veces seguidas —la segunda vez por `mapa.resize()` disparando sus propios eventos de movimiento en bucle con el `ResizeObserver`— y se descartó en vez de seguir persiguiendo el próximo caso borde. Reemplazado por lo mismo que ya usa el botón de búsqueda: se repliega una sola vez a los 10s y no vuelve. Sin lógica reactiva, no hay bucle posible. Verificado en navegador: se oculta una vez y se mantiene estable durante interacción real con el mapa | `lista para revisión` |
